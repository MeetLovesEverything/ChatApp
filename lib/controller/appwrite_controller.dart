...
const String db = "/**/";
const String userCollection = "/**/";
const String chatCollection = "/**/";
...
//to subscribe to real time change
...
Future<String?> updateImageOnBucket(
    {required String imageId, required InputFile image}) async {
  try {
    // to delete the old image
    deleteImagefromBucket(imageId: imageId);
...
Future<bool> deleteImagefromBucket({required String imageId}) async {
  try {
    // to delete the old image
    await storage.deleteFile(bucketId: storageBucket, fileId: imageId);
...
Future sendNotificationtoOtherUser({
  ...
  try {
    ...
    final response = await http.post(
        Uri.parse("https://669530ce26699a1b1e2c.appwrite.global/"),
